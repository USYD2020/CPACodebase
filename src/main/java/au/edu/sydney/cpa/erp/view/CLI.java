...
public class CLI {
    ...
    do {
      selection =
          ViewUtils.displayMenu(
              "Main menu",
              new String[] {
                "Client Actions", "Order Actions", "Report Actions", "Back",
              },
              "Please enter a selection");
  ...
  private static void orderMenu() {
    ...
    do {
      selection =
          ViewUtils.displayMenu(
              "Order menu",
              new String[] {"List all", "View", "Edit", "Remove", "New", "Back"},
              "Please enter a selection");
  ...
  private static void listAllOrders() {
    ...
    for (Integer orderID : orders) {
      System.out.println(
          String.format("%s: $%,.2f", orderID, model.getOrderTotalCommission(orderID)));
    }
  }
  ...
  private static void removeOrder() {
    ...
    while (-1 == orderID) {
      Integer response =
          ViewUtils.getInt("Enter an order id to remove, or blank to return: ", true);
      if (null == response) {
        return;
      }
      if (response < 1) {
  ...
  private static void newOrder() {
    ...
    isScheduled =
        ViewUtils.getBoolean("Is this a scheduled (repeating each quarter) order?: ", false);
    if (null == isCritical || null == isScheduled) return; // just here to keep the linter quiet

    while (-1 == orderType) {
      orderType =
          ViewUtils.getInt(
              "Is this order for regular accounting work (1) or for auditing (2) ?", false);
    ...
    if (isCritical) {
      while (-1 == criticalLoading) {
        criticalLoading =
            ViewUtils.getInt(
                "Please enter the increased commission loading for this critical order in %",
                false);
    ...
    if (isScheduled) {
      while (-1 == numQuarters) {
        numQuarters =
            ViewUtils.getInt("Please enter the number of report batches to schedule", false);
    ...
    if (orderType == 1) {
      while (-1 == maxCountedEmployees) {
        maxCountedEmployees =
            ViewUtils.getInt(
                "What is the threshold for the maximum commission for reports in this order?",
                false);
  ...
  private static void editOrder(int orderID) {
    ...
    while (!"finalise".equals(input)) {
      input =
          ViewUtils.getString(
              "Enter 'list' to list reports, 'view' to view current order contents, "
                  + "'set' to set report quantity, 'finalise' to finalise the order, 'cancel' to remove the order, or 'back' to leave the order to be finished later.",
              false);
      switch (input) {
        case "list":
          listAllReports();
        ...
        case "set":
          ...
          Integer reportEmployeeCount =
              ViewUtils.getInt(
                  "Please enter the number of employees covered by this report, 0 to remove, or blank to cancel",
                  true);
        ...
        case "finalise":
          Boolean customPriority =
              ViewUtils.getBoolean("Would you like to use a custom invoicing method list?", false);
        ...
        case "back":
          System.out.println(
              "Order " + orderID + " will be left unfinished. Use this order id to return later.");
          return;
        default:
  ...
  private static List<String> getCustomPriorityList() {
    ...
    while (!"end".equals(response)) {
      response =
          ViewUtils.getString(
              "Enter a contact method, 'end' to finish, blank to list available methods", true);
      if ("".equals(response)) {
  ...
  private static void showReportDetails() {
    ...
    while (-1 == reportID) {
      Integer response =
          ViewUtils.getInt("Enter a report type to view, or blank to return: ", true);
      if (null == response) {
        return;
      }
